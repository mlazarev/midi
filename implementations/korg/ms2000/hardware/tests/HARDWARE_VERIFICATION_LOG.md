...
|------|----------|------------|-----------------|----------|--------|--------|-------|
| TBD  | 1.x      | `edge_case_pan_center.json` | Amp panpot | 0 → byte 64 → synth CNT | — | ☐ | Verify after sending via `tools/send_sysex.py`. |
...
2. Transmit via `python3 tools/send_sysex.py --file <patch.syx> --out "MS2000"` (substring match your interface).
